...
import org.gerryrig.SharedClasses.Commands.DrawRectangleCommand;
import org.gerryrig.SharedClasses.Commands.ScrollingTextCommand;
import org.gerryrig.SharedClasses.Commands.TextCommand;
import org.gerryrig.SharedClasses.Commands.TextShiftingCommand;
import org.gerryrig.SharedClasses.Commands.ValueDisplayCommand;
import org.gerryrig.SharedClasses.Drawing.BitmapFont;
import org.gerryrig.SharedClasses.Drawing.BitmapFontManager;
...
public class RobotContainer {
    ...
    public RobotContainer() {
        m_ledSubsystem = new LEDPanelSubsystem(Constants.LEDConstants.kMatrixWidth,
                Constants.LEDConstants.kMatrixHeight, Constants.LEDConstants.kLEDPWMPin);
        m_ledSubsystem.setBrightnessOverride(0.10);
        ...
        // m_ledSubsystem.setDefaultCommand(new
        // RainbowCommand(m_ledSubsystem).ignoringDisable(true));

        File deployDirectory = Filesystem.getDeployDirectory();
        m_fontManager.addFont(new BitmapFont(new File(deployDirectory, Constants.Fonts.kSmallFontFile)));
        // m_fontManager.addFont(new BitmapFont(new File(deployDirectory,
        // Constants.Fonts.kMediumFontFile)));
        m_fontManager.addFont(new BitmapFont(new File(deployDirectory, Constants.Fonts.kLargeFontFile)));
        // m_fontManager.addFont(new BitmapFont(new File(deployDirectory,
        // Constants.Fonts.kMediumAltFontFile)));

        // Note: Subsystem default commands must require their subsystem. So at least
        // one of the commands in a
        ...
        if (false) {
            ...
            m_upperHalf.setDefaultCommand(
                    new TextShiftingCommand("Janksters!",
                            () -> m_encoder.getAbsolutePosition().getValueAsDouble() / 0.5, new Point(0, 0), smallFont,
                            Color.kRed, m_upperHalf)
                            .ignoringDisable(true));
            m_lowerHalf.setDefaultCommand(
                    new ValueDisplayCommand(() -> m_encoder.getAbsolutePosition().getValueAsDouble(), new Point(0, 0),
                            smallFont, Color.kBlue, m_lowerHalf)
                            .ignoringDisable(true));
        }
    }
    ...
    public Command fontTestCommand() {
        ...
        return new SequentialCommandGroup(
                new DrawRectangleCommand(new Rectangle(0, 0, m_ledSubsystem.width(), m_ledSubsystem.height()),
                        Color.kBlack, m_ledSubsystem),
                ...
                new ScrollingTextCommand("Watch \"Jeopardy!\", Alex Trebek's fun TV quiz game.", new Point(0, 1), 16,
                        largeFont, Color.kBlue, m_ledSubsystem),
                new DrawRectangleCommand(new Rectangle(0, 0, m_ledSubsystem.width(), m_ledSubsystem.height()),
                        Color.kBlack, m_ledSubsystem),
                new ParallelDeadlineGroup(
                        // To have two different regions scrolling simultaneously, we need to use the
                        // upper/lower half subsystems
                        // because two commands can't use the same subsystem at the same time
                        new ScrollingTextCommand("Watch \"Jeopardy!\", Alex Trebek's fun TV quiz game.",
                                new Point(0, 1), 16, smallFont, Color.kGreen, m_lowerHalf),
                        new ScrollingTextCommand("The quick brown fox jumps over the lazy dog!", new Point(0, 1), 50,
                                smallFont, Color.kRed, m_upperHalf).repeatedly()))
    ...
    private void configureBindings() {
        ...
        // m_joystickController.button(1).whileTrue(new TextCommand("Hello World!",
        // m_ledSubsystem));
    }
